...
class Quidax {
  ...
  constructor(api_key: string, merchant_id: string) {
  ...
  async create_subAccount(email: string, firstname: string, lastname: string, phone_number: string) {
    const body = {
      email,
      firstname,
      lastname,
  ...
  async create_payment_address(user_id: string, currency: string, network: string) {
  ...
  async fetch_payment_address(user_id: string, currency: string) {
    try {
  ...
  async fetch_payment_addresses(user_id: string, currency: string) {
    try {
  ...
  async fetch_user_wallet(user_id: string, currency: string) {
    try {
  ...
  async fetch_all_user_wallets(user_id: string) {
    try {
  ...
  async create_a_withdrawal_to_merchant(amount: string, user_id: string) {
    const currency = 'ngn';
  ...
  async create_instant_order(user_id: string, currency: string, volume: number) {
    const body = {
  ...
  async confirm_instant_order(user_id: string, instant_order_id: string) {
    try {
      const response = await axios.post(
  ...
  async fetch_instant_order(user_id: string, instant_order_id: string) {
    try {
